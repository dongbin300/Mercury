...
namespace Mercury.Extensions
{
    ...
    public static class IndicatorExtension
    {
        ...
        public static IEnumerable<EmaResult> GetVolumeEma(this IEnumerable<Quote> quotes, int period)
        {
            var result = new List<EmaResult>();
        ...
        public static IEnumerable<DemaResult> GetDema(this  IEnumerable<Quote> quotes, int period)
        {
            var result = new List<DemaResult>();

            var values = quotes.Select(x => (double)x.Close).ToArray().ToNullable();
            var dema = ArrayCalculator.Dema(values, period);
            for (int i = 0; i < dema.Length; i++)
            {
                result.Add(new DemaResult(quotes.ElementAt(i).Date, (decimal?)dema[i]));
			}

            return result;
		}

		public static IEnumerable<DemaSlopeResult> GetDemaSlope(this IEnumerable<Quote> quotes, int period)
		{
			var result = new List<DemaSlopeResult>();

			var values = quotes.Select(x => (double)x.Close).ToArray().ToNullable();
			var demaSlope = ArrayCalculator.DemaSlope(values, period);
			for (int i = 0; i < demaSlope.Length; i++)
			{
				result.Add(new DemaSlopeResult(quotes.ElementAt(i).Date, (decimal?)demaSlope[i]));
			}
		...
		public static IEnumerable<CciVolatilityThresholdResult> GetCciVolatilityThreshold(this IEnumerable<Quote> quotes, int cciPeriod = 20, int volatilityPeriod = 20, double stdevMultiplier = 2, double minThreshold = 50, double maxThreshold = 200)
		{
			var result = new List<CciVolatilityThresholdResult>();

			var high = quotes.Select(x => (double)x.High).ToArray();
			var low = quotes.Select(x => (double)x.Low).ToArray();
			var close = quotes.Select(x => (double)x.Close).ToArray();
			var cciVolatilityThreshold = ArrayCalculator.CciVolatilityThreshold(high, low, close, cciPeriod, volatilityPeriod, stdevMultiplier, minThreshold, maxThreshold);
			for (int i = 0; i < cciVolatilityThreshold.Length; i++)
			{
				result.Add(new CciVolatilityThresholdResult(quotes.ElementAt(i).Date, (decimal?)cciVolatilityThreshold[i]));
			}
