...
{
	public class Ci25(string reportFileName, decimal startMoney, int leverage, MaxActiveDealsType maxActiveDealsType, int maxActiveDeals)
	: Backtester(reportFileName, startMoney, leverage, maxActiveDealsType, maxActiveDeals)
	{
		...
		protected override void LongEntry(string symbol, List<ChartInfo> charts, int i)
		{
			...
			if (c2.Cci < -100 && c1.Cci > -100
				&& c1.Quote.Close > c1.IcLeadingSpan1
				&& c1.Quote.Close > c1.IcLeadingSpan2)
			{
		...
		protected override void LongExit(string symbol, List<ChartInfo> charts, int i, Position longPosition)
		{
			...
			if (c1.Quote.Close < c1.IcLeadingSpan1
				&& c1.Quote.Close < c1.IcLeadingSpan2)
			{
